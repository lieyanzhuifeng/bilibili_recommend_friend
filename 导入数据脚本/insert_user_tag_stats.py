...
def initialize_user_tag_stats():
    """从多张表初始化user_tag_stats表"""
    try:
        # 连接数据库
        ...
        with connection.cursor() as cursor:
            # 从多张表统计数据并插入到user_tag_stats
            print("📊 从多张表统计数据...")

            sql = """
                  INSERT INTO user_tag_stats (userID, tagID, totalWatchDuration, uniqueVideos)
                  SELECT wl.userID, \
                         tv.tagID, \
                         SEC_TO_TIME(SUM( \
                                 CASE \
                  ...
                  FROM user_watch_log wl
                           JOIN tag_video tv ON wl.videoID = tv.videoID
                  GROUP BY wl.userID, tv.tagID \
                  """
            ...
            print("\n📋 数据预览 (前10条):")
            cursor.execute("""
                           SELECT tagStatID, userID, tagID, totalWatchDuration, uniqueVideos
                           FROM user_tag_stats
                           ORDER BY TIME_TO_SEC(totalWatchDuration) DESC LIMIT 10
                           """)
            preview_data = cursor.fetchall()

            print(f"{'tagStatID':<8} {'userID':<8} {'tagID':<8} {'总时长':<15} {'唯一视频数':<12}")
            print("-" * 60)
...
def check_user_tag_stats_status():
    """检查user_tag_stats表状态"""
    try:
        connection = pymysql.connect(
            host='47.100.240.111',
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT COUNT(*) as count FROM user_tag_stats")
            count = cursor.fetchone()[0]

            if count > 0:
                print(f"✅ user_tag_stats表已有 {count} 条记录")
...
def main():
    ...
    while True:
        print("\n" + "=" * 60)
        print("🏷️ User Tag Stats 初始化工具")
        print("=" * 60)
        print("1. 检查表状态")
        print("2. 初始化数据（从多张表统计）")
        print("0. 退出")
        print("-" * 60)
        ...
        if choice == '1':
            check_user_tag_stats_status()

        elif choice == '2':
            confirm = input("⚠️ 确定要从多张表初始化数据吗？(y/N): ").strip().lower()
            if confirm == 'y':
                initialize_user_tag_stats()
            else:
                print("❌ 操作已取消")
